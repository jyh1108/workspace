...
// m3 is not defined (정의 되지 않음 == 존재하지 않거나 접근 불가능)
// console.log("함수 내부에서 선언된 (함)num3 : " + num3);
...
// -> error가 발생하지 않고 underFined로 출력됨
console.log(test);
...
function typeTest(){
    ...
    box.innerHTML += "<br>name :" + phone + " / " + typeof phone;

    // number
    const age =20; //정수
    const height = 178.9 //실수
    box.innerHTML += "<br>name :" + age + " / " + typeof age;
    box.innerHTML += "<br>name :" + height + " / " + typeof height;

    //boolean
    const isTrue = true;
    box.innerHTML += "<br>name :" + isTrue + " / " + typeof isTrue;
    ...
    box.innerHTML += "<br>name :" + arr + " / " + typeof arr;

    //java 객체 --> class에 작성된 내용대로 메모리(heap)에 할당 
    // js의 객체는 {K:V} map형식으로 작성
    //  ** key는 무조건 string으로 고정 **
    const user = {"id": "user01",  'pw':'pass01', address:'서울시 종구'}
    box.innerHTML += "<br>name :" + user + " / " + typeof user;
...
// 객체 v 출력 방법 (2)
    ...
    box.innerHTML += "<br>name :" + sumFn + " / " + typeof sumFn;
...
function doubleFn(fn){
    ...
    // 전달 받은 함수의 결과를 2배로 반환하는 함수 2배로 반환하는 함수 
    return fn(1,2)* 2;
