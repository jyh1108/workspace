...
document.querySelector("#btn2").addEventListener("click",function(e){
    ...
    //부동 소수점(부정확, 용량 작은) -> 고정 소수점(정화그 용량큼)변경

    //숫자.toFixed(보여질 소수 자릿수)
    // - 주어진 숫자를 매개변수에 작성된 숫자만큼의
    // 소수점 자릿수로 표현될 수 있도록
    // 바로 다음 소수 자리에서 반올림해서 문자열로 반환
    console.log((3.45).toFixed(2));
    console.log((3.45).toFixed(1));
    console.log((3.45).toFixed(0));

    //Math.random() -> 0.0 <= x <10 실수 반환

    //버튼 클릭 시 마다 버튼 배경색 변경
