...
//	private List<Student> studentList = new ArrayList<Student>();
	...
	public Student removeStudent(int index) {
		...
		//boolean remove(E e) : List에서 E와 일치하는 요소를 찿아서
		//					  : 있으면 제거하고 true
//							  : 없으면 false 반환
	...
	/** 학년 별 조회
	 * @param input1
	 * @return list : list<Student>  학년별 학생 조회
	 */
	public List<Student> selectGrade(int input1) {
		...
		for(Student s : studentList) {
			if(s.getGrade() == input1)list.add(s);
		}
		return list;
	}
	...
	/** 성별 조회
	 * @param input2
	 * @return list : list<Student> 성별로 학생 조회
	 */
	public List<Student> selectGender(char input2) {
		List<Student> list = new ArrayList<Student>();
										
		for(Student s : studentList) {
			if(s.getGender()==input2)list.add(s);
		}
		return list;
	}
	...
	public List<Student> sortScore() {
		...
		// - Collection 클래스 : 컬렉션에 도움되는 유용한 기능을 모은 클래스
