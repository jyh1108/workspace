...
// 값/ 입력 / 출력용 클래스 
// -> 사용자가 맞닿는(접점) 클래스 == User Interface (UI)
public class EmployeeView {
	...
	public void displayMenu() {
		...
		do {
			try {
				...
				System.out.println("3. 이름에 글짜가 포함된 사원 조회");
	...
	private void selectName() {
		...
		try {
			List<Employee> empList = service.selectName(input2);

			if (empList.isEmpty()) {// 조회 결과 없는경우
				System.out.println("[입력된 글자를 포함하는 이름이 존재하지 않습니다.]\n");
				return;
			...
			for (Employee emp2 : empList) {
				System.out.printf("%d / %s / %s /%s / %s \n", emp2.getEmpId(), emp2.getEmpName(),
						emp2.getDepartmentTitle(), emp2.getJobName(), emp2.getPhone());
			}
		} catch (SQLException e) {
			System.out.println("\n[글자가 이름에 포함된 사원 조회중 예외 발생.]\n");
			e.printStackTrace();
		}
	}
	...
	private void selectSalary() {
		...
		try {
			...
			// 조회 결과가 있을 경우 (n행)
			for (Employee emp : empList) {
				System.out.printf("%d / %s /%s /%d \n", emp.getEmpId(), emp.getEmpName(), emp.getJobName(),
						emp.getSalary());
			}
		} catch (SQLException e) {
	...
	private void insertEmployee() {
		...
		Employee emp = new Employee(empName, empNo, email, phone, salary, deptCode, jobCode, salLevel, bonus,
				managerId);

		// 사원 정보를 DB에 삽입하는 서비스 호출 후 결과 반환받기
		try {
