...
/*  정규 표현식(Regular Expression)
    ...
    *** JS 정규표현식 객체 생성 방법  ***
    ...
    1.const regEx = new RegExp("정규표현삭");
    2. const regEx = /정규표현식/;

    *** 정규표현식 객체가 제공하는 메서드(함수)
    1. regEx.text(문자열)
        -> 문자열이 정규표현식 패턴에 부합하면 true, 아니면 false를 반환
    ...
    2. regEx.exec(문자열)
        ...
        [0-9]  : 0부터 9까지 모든 숫자
        [ㄱ-힣] : ㄱ부터 힣까지  모든 한글
        [가-힣] : 가부터 힣까지(자음만, 모음만 있는경우 제외)  모든 한글
        [a-z] : 모든 영어 소문자
        [A-Z] : 모든 영어 대문자
...
memberEmail.addEventListener("input", () => {
    ...
    if (memberEmail.value.trim().length == 0) {
        ...
        //confirm,error 클래스 지우기 [검정 글씨적용]
        emailMessage.classList.remove("confirm", "error");

        checkObj.memberEmail = false; //빈칸 == 유효x
        return;
    }


    //정규 표현식을 이용해 유효한 형식이지 판별 
    ...
    if (regEx.test(memberEmail.value)) { // 유효한


        /*******************************************/
        /* fetch() API를 이용한 ajax(비동기 통신) */

        //GET 방식 ajax 요청 (파라미터를 쿼리스트링으로....)
        fetch('/dupCheck/email?email=' + memberEmail.value)
            ...
            .then(count => {
                ...
                if (count == 0) {
                    ...
                    checkObj.memberEmail = false; // 유효x
        ...
        emailMessage.innerHTML = "유효한 형식이 유효하지 않습니다.";
    ...
    if (regEx.test(memberPw.value)) {
        ...
        if (memberPwConfirm.value.trim().length == 0) {
            ...
            if (memberPw.value == memberPwConfirm.value) {
                ...
                pwMessage.classList.add("confirm");
                pwMessage.classList.remove("erorr");
...
memberTel.addEventListener('input', () => {
    if (memberTel.value.trim() == '') {
        ...
        telMessage.classList.remove("confirm", "erorr");
...
document.getElementById("signUpFrm").addEventListener("submit", e => {
    ...
    for (let key in checkObj) {
        if (!checkObj[key]) { // 각 key에 대한 value(true/false)를 얻어
            // false인 경우 == 유호하지 않다.
...
memberNickname.addEventListener("input", () => {
    ...
    if (memberNickname.value.trim() == '') {
        ...
        nickMessage.classList.remove("confirm", "erorr");
        ...
        fetch("/dupCheck/nickname?nickname=" + memberNickname.value)
            ...
            .then(count => {
                if (count == 0) { //중복이 아닌 경우
                    ...
                    nickMessage.innerText = "이미 사용죽인 닉네임 입니다";
