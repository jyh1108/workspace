...
-- EMPLOYEE 테이블에서 
-- 성별과, 성별 별 급여 평균(내림),급여 합, 인원 수 조회
-- 인원 수 내림차순 정렬
...
-- WHERE + GROUP BY 
--> WHERE절이 GROUP BY 절보다 우선 순위가 빠르다!
...
-- 부서별 급여가 300백만 이상인 사원들의 평균 급여을 조회
SELECT DEPT_CODE, AVG(SALARY)
FROM EMPLOYEE
...
--EMPLOYEE 테이블에서
-- 부서별 70년대생의 급여 평균이 300만 이상인 부서를 찿아 
-- 부서코드, 평균 급여(소수점 내림)을 부서코드 내림 차순 조회

SELECT DEPT_CODE , FLOOR(AVG(SALARY))
FROM EMPLOYEE  
WHERE SUBSTR(EMP_NO, 1 ,2) < 80 AND SUBSTR(EMP_NO, 1 ,2) >= 70
...
-- 2게 이상의 SELECT 결과(RESULT SET)을 이용해
-- 하나의 결과를 조회하는 연산자

--조건에 따른 SELECT 결과가 다른 경우 
-- 많은 SELECT를 한번에 초회할 때 유용
...
-- UNION ALL 화인 
-- 부서코드가 'D5' 또는 급여 300만 초과인 사원 조회 (증복 O )
SELECT EMP_ID , EMP_NAME , DEPT_CODE ,SALARY 
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
UNION ALL 
