...
SELECT EMP_NO, EMP_NAME ,JOB_CODE ,DEPT_CODE, HIRE_DATE
FROM EMPLOYEE 
JOIN JOB USING(JOB_CODE)
...
SELECT EMP_ID, EMP_NAME ,JOB_NAME,FLOOR(MONTHS_BETWEEN(SYSDATE, TO_DATE(SUBSTR(EMP_NO, 1, 6))) /12)나이
, (SALARY+(SALARY*NVL(BONUS,'1')))*12
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
WHERE (JOB_NAME ,FLOOR(MONTHS_BETWEEN(SYSDATE, TO_DATE(SUBSTR(EMP_NO, 1, 6))) /12)) =
(SELECT JOB_NAME  ,MIN(FLOOR(MONTHS_BETWEEN(SYSDATE, TO_DATE(SUBSTR(EMP_NO, 1, 6))) /12))
						FROM EMPLOYEE
						GROUP BY JOB_NAME)
ORDER BY 나이 DESC;