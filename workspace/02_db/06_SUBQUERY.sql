...
/*  서브쿼리 유형
    ...
    - 다중열 서브쿼리 : 서브쿼리의 SELECT 절에 자열된 항목수가 여러개 일 때(행, 열N)
...
LEFT JOIN DEPARTMENT ON(DEPT_NAME - DEPT_ID)
WHERE SELARY >= (SELECT AVG(SALARY) FROM EMPLOYEE)
ORDER BY JOB_CODE;
...
SELECT EMP_ID, EMP_NAME , DEPT_TITLE, JOB_NAME, '사수'  AS 구문
FROM EMPLOYEE 
LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
JOIN JOB USING(JOB_CODE)
...
SELECT EMP_ID, EMP_NAME , DEPT_TITLE, JOB_NAME, '사원'  AS 구문
FROM EMPLOYEE 
LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
JOIN JOB USING(JOB_CODE)
...
-- EXISTS : 서브쿼리에서 조회된 결과와 일치하는 해이
--          메인 쿼리에 하나라도 있으면 조회 결과에 포함
--         -> 서브쿼리 조회 결과가 1행 이상이면 메인 쿼리 행을 결과에 포함
SELECT EMP_ID , EMP_NAME , DEPT_TITLE, MANAGER_ID
FROM EMPLOYEE MAIN
LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
WHERE EXISTS (SELECT EMP_ID FROM EMPLOYEE
			 WHERE EMP_ID = MAIN.MANAGER_ID);
...
SELECT DEPT_CODE ,MIN(HIRE_DATE) FROM EMPLOYEE 
WHERE ENT_YN = 'N'
GROUP BY DEPT_CODE )
ORDER BY HIRE_DATE ;
...
WHERE HIRE_DATE 
	=(
		...
		WHERE (ENT_YN ='N' AND SUB.DEPT_CODE = MAIN.DEPT_CODE )-- NULL X인경우 
		OR (ENT_YN = 'N'
...
SELECT DEPT_ID, DEPT_TITLE,(SELECT EMP_NAME FROM EMPLOYEE WHERE EMP_ID = '200')
FROM DEPARTMENT ;
...
-- 3) ROWNUM을 WHERE애 사용하기 
SELECT EMP_NAME , ROWNUM
FROM EMPLOYEE
WHERE ROWNUM <= 5;


--4) 1,2,3 번을 토대로 급여 상위5명 조회 시도
SELECT EMP_NAME, SALARY , ROWNUM
FROM EMPLOYEE 
WHERE ROWNUM <=5
ORDER BY SALARY DESC;
--> EPLOYEE 테이블 위에서 부터 5명을 가지고 급여 내리차순 조회


--문제 원인 : SELECT 해석 순서상
--          ORDER BY가 가장 마지막으로 해석되기 때문에
--			급여 상위 5명이라는 의도와 맞지 않는 상태로 초화됨.
			
-- 해결방법 : 인라인뷰(FROM절에 작성되는 서브 쿼리를 )
...
-- 해결2) 해결 1의 조회 결과를 FROM절에 작성하여 상위 5열만 절명
...
WITH TOP_SALARY AS (SELECT EMP_NAME, SALARY0
FROM EMPLOYEE
ORDER BY SALARY DESC)
SELECT ROWNUM, EMP_NAME, SALARY
FROM TOP_SALARY;
