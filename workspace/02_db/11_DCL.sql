 ...
 *  - 사용자(USER)에 DB 또는 DB객체에 대한 접근 권한 을 
...
ALTER USER jyh_sample DEFAULT TABLESPACE SYSTEM;
...
-- 1. kh계정의 EMPLOYEE 테이블 조화
SELECT * FROM kh.jyh.EMPLOYEE;
...
--> sample 계정에 시쿼스, 프로시저, 클러스, 인덱스타입, 연산자, 타입, 트리거, 테이블 생성 권한부여
...
CREATE TYPE
CREATE TRIGGER
CREATE TABLE TO jyh_sample;

--> ROLE을 이용해서 간단히 부여 /회수
GRANT RESOURCE TO jyh_sample;
REVOKE RESOURCE FROM jyh_sample;
...
-- 사용자 에게 부여된 권한 조회 
SELECT * FROM JYH_ROLE_PRIVS
WHERE GRANTEE ='JYH_SAMPLE';
...
GRANT CONNECT, RESOURCE, CREATE VIEW TO jyh_sample;
										--(계정명)