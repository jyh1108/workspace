...
--／＊　ＳＥＬＥＣＴ（조회，ＤＱＬ또는　ＤＭＬ）
--＊　－－＞조회된　행들의　집합을　＂ＲＥＳＵＬＴ　ＳＥＴ＂이라고　한다．
--＊
--＊－　ＲＥＳＵＬＴ　ＳＥＴ은　０개　이상의　행이　포함될　수　있다．
--＊　－＞　왜　０게　이상？？　조건에　맞는　행이　없을　수도　있기　때문에．．．
--＊
--
--＊［작성법］
--＊
--＊　ＳＥＬＥＣＴ　컬럼명 ｜＊（모든　컬럼）
--＊　ＦＲＯＭ　테이블명
--＊　ＷＨＥＲＥ　조건식：　－－　조건에　맞는　행만　조회
--＊
--＊－－　ＥＭＰＬＯＹＥＥ　테이블에서　모든　해으　모든　컬럼을　조회
--＊－－　（ＷＨＥＲＥ절　없음　＝＝　모든　행）
--＊－－　＇＊＇　：　ＡＬＬ，　모두，전부
--＊／
SELECT  EMP_NAME FROM EMPLOYEE;
SELECT  EMP_ID, EMP,NAME PHONE FROM EMPLOYEE;
--－－　ＤＥＰＡＲＴＭＥＮＴ　테이블의　모든　행，　열　조회
SELECT  * FROM DEPARTMENT;

--－－ＤＥＰＡＲＴＭＥＮＴ　테이블에서　부서명만　조회
--－－＞　테이블　정보에서　컬럼명，　ＣＯＭＭＥＮＴ확인
SELECT  DEPT_TITLE FROM DEPARTMENT;
--－－－－－－－－－－－－－－－－－－－－－－－－－－

--<컬럼 값 산술 연산>

-- 컬럼 값 : 행과 열이 교차되는 테이블의 한 칸 (== 한 셀)에 작성된 값 (DATE)

-- SELECT 문 작성 시 컬럼명에 산술 연산을 작성하면 
-- 조회되는 결과 컬럼 값에 산술 연산이 반영된다.

--EMPLOYEE 테이블에서 모든 사원의 사번, 이름, 급여, 급여 + 100만을 조회
SELECT EMP_ID, EMP_NAME, SALARY, SALARY + 10000000
FROM EMPLOYEE;
...
--별칭 지정 방법
-- 1) 컬럼명 AS 별칭 : 문자O,뛰어쓰기 X, 특수문자 X
-- 2) 컬럼명 AS "별칭" : 문자O,뛰어쓰기 O, 특수문자 O
-- 3) 컬럼명 별칭 : 문자O,뛰어쓰기 X, 특수문자 X
-- 4) 컬럼명 "별칭" : 문자O,뛰어쓰기 O, 특수문자 O
...
--<DISTINCT> (별개의, 따로따로)
-->조회 시 시정된 컬럼에 중복 값을 한 번만 표시할 때 사용

--EMPLOYEE 테이블에서 현재 사원들이 속해있는 부서 코드화인.
...
--컬럼명 BETWHEEN A AND B : 컬럼 값이 A이상 B 이하인 경우 TRUE 
SELECT *
FROM EMPLOYEE
...
-- 컬럼명 IN (값1,값2,값3, .....) : 컬럼 값이 () 안에 값이 포함된 행은 TRUE
...
-- 컬럼명 NOT IN (값1,값2,값3, .....) : ()내 값이 제외한 행 TRUE
...
-- IS NOT NULL : 컬러 값이 NULL이 아닌 경우 TRUE
...
/* 	LIKE
*
* - 비교하려는 값이 특정한 패턴을 만족 시 키면 조회하는 연산자 
*
* [작성법]
* WHERE 컬럼명 LIKE '패턴'
*
...
--EMPLOYEE 테이블에서 성이 '하' 씨인 사원의 사번, 이름 조회
SELECT EMP_ID, EMP_NAME 
FROM EMPLOYEE
...
--				-> '#' 뒤에 한 글자 (_)를 일반 문자로 벗어남 


-------------------------------------------------------

-- <WHERE절 날짜(시간) 비교>
-- EMPLOYEE테이블에서 입사일이(고용일)이
-- '1990/01/10' ~ '2000/12/31' 사이의 사원의
-- 사번, 이름, 고용일 조회
...
FROM EMPLOYEE
ORDER BY PHONE ASC /* NULLS LAST */NULLS FIRST;
				-- 오름차순 기본값
...
/* <정렬 중첩>
 * 
 * - 큰 분류를 먼저 정렬하고, 내부 분류ㄹ,ㄹ 다음에 정렬하는 방식
 */ 
