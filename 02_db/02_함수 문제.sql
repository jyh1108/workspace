...
-----------------------------------------------
-- INSTR(문자열 | 컬럼명, '찿을 문자열'[, 찿을 시작 위치[, 순번] ])
-- 찿을 시작 위치부터 지정된 순번째에 찿을 문자열의 시작 위치(인덱스) 반환

--문자열에서 맨 앞에있는 B의 위치 조회
SELECT 'AABAACAABBAA', INSTR('AABAACAABBAA','B') FROM DUAL;

--문자열을 5번째 부터 검색하여 처음 찿는 B의 위치 조회
SELECT 'AABAACAABBAA', INSTR('AABAACAABBAA','B',5) FROM DUAL;
					 	--    123456789

--문자열을 5번째 부터 검색하여 두번째로 찿는 B의 위치 조회
...
-- 주어진 문자열에 앞|뒤|양쪽에 존재하는 지정된 무자열을 제거
...
-----------------------------------
--REPLACE(문자열 | 컬럼명, 찿을 문자열, 바꿀 문자열)
-- 찿을 문자열을 바꿀 문자열로 변경하여 문자열 반환
...
SELECT 123.456,
	...
	ROUND(123.456, -1), -- 소수점 -1번째 자리 -> 정수 1의 자리 반올림
	ROUND(123.456, -2)  -- 소수점 -2번째 자리 -> 정수 10의 자리 반올림
FROM DUAL;
...
-- EXTRACT (YEAR | MONTG | DAY DROM 날짜)
-- 날짜에서 년 | 월 | 일을 추출하여 정수로 반환

-- EMPLOYEE 테이블에서
-- 각 사원의 이름, 입사 년, 월, 일 조회
-- 단, 2000년 이상 입시 사원만 조회
SELECT EMP_NAME 이름,
...
-- TO_DATE(문자열, 숫자 [, 포맷] )
--> 문자열이나 숫자를 지정된 포맷에 날짜 형식으로 해석하여 DATE 타입으로 반환

SELECT '2023-03-07', TO_DATE('2023-03-07') FROM DUAL; 

SELECT '2023-03-07', TO_DATE('2023년 03월 07일', 'YYYY"년" MM"월" DD"일"') FROM DUAL; 
...
--50미만 : Y,R 둘다 앞부분에 현재 세기 (21C == 2000년대)세기를 적용 
-- 50이상 : Y == 현재 세기(2000년대)
--		   R == 이전 세기(1900년대)
...
-- DECODE( 계산식 | 컬럼명, 조건1, 결과, 조건2, 결과2 ....[, 아무것도 일치 안할때])
-- 비교하는 식 또는 컬럼의 갑이 일치하는 조건이 있으면 
-- 해당 조건의 결과를 반환 
...
-- 지급코드가 'J6'인 직원의 급여 + 급여의 15%
-- 지급코드가 'J5'인 직원의 급여 + 급여의 20%
-- 나머지 직급코드의 직원은 급여 + 급여의 5%
-- 사원명, 직급코드, 기존급여, 직급급여, 조회
...
--EMPLOYYEE 테이블에서 사번, 이름, 급여, 구분을 조회
-- 구분은 받는 급여에 따라 초급, 중급, 고급으로 조회
-- 급여 500만 이상 = '고급'
-- 급여 300만 이상 ~ 500만 미만 = '중급'
-- 급여 300미만 = '초급'

-- 단, 부서코드가 D6인 사원만 직급코드 오림차순으로 조회
...
--EMPLOYEE 테이블의 남자 사원수 조회
SELECT COUNT(*) FROM EMPLOYEE
WHERE SUBSTR(EMP_NO,8,1) = 1; 
--EMPLOYEE 테이블의  여자 사원수 조회
SELECT COUNT(*) FROM EMPLOYEE
WHERE SUBSTR(EMP_NO,8,1) = 2; 
--EMPLOYEE 테이블의 남자 사원수, 여자 사원수 조회
