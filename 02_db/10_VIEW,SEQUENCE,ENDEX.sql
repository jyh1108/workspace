...
* 
*  ** VIEW 작성법 ** :
*  CREAT [OR REPLACE) [FORCE | MOFORCF] VIEW 뷰이름
*  AS 서브쿼리(SELECR문)
* [WITH CHECK OPTION]
* [WITH READ ONLY]
* 
...
-- D0    NULL    L3
---> 원본 테이블에 삽입됨을 확인
----> INSERT 구문에 포합되지 않은 DEPT_TITLE 컬럼은
-- NULL이 삽입되어 있다!(중요)
...
CREATE OR REPLACE VIEW V_DCPY2
AS SELECT DEPT_ID, DEPT_TITLE,LOCATION_ID FROM DEPARTMENT2;

INSERT INTO V_DCOPY2
VALUES('0','기획팀','L3'); -- 오류없이 삽입됨
 ...
 *  				(INCREMENT_BY 만큼 증가된 수)
 * 					단, 생성 후 처음 호출된 시퀀스인 경우
 * 		       		START WITH에 작성된 값이 반환됨.
 * 2)시퀀스명. CURRCVAL : 현재 시퀀스 번호를 얻어옴
...
-- SEQ_TEST_NO의 증가값을 1ㅡ 최대값 155로 수정
...
--TB_INDEX_TEST 테이블에 샘플 데이터 100만게 삽입
-- (PL/SQL 사용)
...
WHERE TEST_ID = 'TEST500000'; --20/14/16/15/14MS
...
--인덱스 생성 후 조회
SELECT *FROM TB_INDEX_TEST
WHERE TEST_ID = 'TEST500000'; --15/4/2/1/1 MS
...
SELECT *FROM TB_INDEX_TEST
WHERE TEST_ID = 'TEST500000'; --55/27/18
