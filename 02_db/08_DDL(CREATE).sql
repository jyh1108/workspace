...
MEMBER_NAME VARCHAR2(15), -- 한글 2글자~ 5글자 (3BYTE)
MEMBER_SSN CHAR(14),      --990123-1234567 *(항상14글자)
ENROLL_DATE DATE DEFAULT SYSDATE -- 가입일
);

--DEFAULT(기본겂) : INSERT/UPDATE 시
--				해당 컬럼의 값을 작성하지 않거나
--				DEFAULT로 작성하면
--				DEFAUTL 오른쪽에 작성된 값에 컬럼에 반영됨.
...
--> 문재 해결 방법 : CHAR, VARCHAR2 타입으로 컬럼 타입을 지정 
--------------------------------------------------------------------------------------------------------------------
...
CREATE TABLE USER_USED_UK2(
    ...
    --테이블 레벨 복합기 설정
    , CONSTRAINT USER_IN_NAME_UK UNIQUE(USER_ID, USER_NAME)
);
...
--    USER_NO NUMBER CONSTRAINT USER_NO_PK PRIMARY KEY, -- 컬럼 레벨(제약조건명 X)
...
--컬럼명 자료형(크기)  [CONSTRAINT 이름] FOREIGN KEY (적용할컬럼명) REFERENCES 참조할테이블명 [(참조할컬럼)]
);
...
--> NULL (외부 테이블에 참조할 값이 없다, 참조 X)사용 가능.
...
WHERE GRADE_CODE = 20;
--> GRADE_CODE =20번은 자식 테이블에서 참조하고 있지 않으므로
-- 삭제가 제한되지 않음
DELETE FROM USER_GRADE
WHERE GRADE_CODE = 10;
...
-- 부모 테이블인 USER_GRADE2에서 GRADE_CODE =10 삭제 --중요
...
-- 3) ON DELETE CASCADE : 부모키 삭제시 자식키도 함께 삭제됨  --중요
...
COMMENT ON COLUMN USER_ND IS '회원번호'
COMMENT ON COLUMN USER_ID IS '회원읾'
COMMENT ON COLUMN USER_PWD IS '비빌번호'
COMMENT ON COLUMN USER_PNO IS '폰번호'
COMMENT ON COLUMN USER_ADDRESS IS '주소지'
COMMENT ON COLUMN USER_ID IS '회원번호'
...
--AS 서브쿼리; -- 서브쿼리의 결과(RESULT SET)의 형태로 태아블 생성
