...
WHERE DEPT_CODE =DEPT_ID(+);
-- NULL = D1 ~ D9(+)
-- 왼쪽테이블  컬럼의 값과, 오르쪽 테이블 컬럼의 값이 일치하지 않아도
-- 억지로 축하해라...
...
-- EMPLOYEE : DEPT_ CODE CHAR(@) D1 ~ D9 , NULL
-- DEPARTMENT : DEPT_ ID CHAR(@) D1 ~ D9 
---------------------------------------------------------------------------------------------------------------
...
--연봉에 보너스포인트를 적용하시오.
SELECT EMP_NAME , JOB_NAME,SALARY , SALARY, SALARY  * (1+ NV)
FROM EMPLOYEE 
NATURAL JOIN JOB
JOIN SAL_GRADE USING(SAL_GRADE)
